// Arrears Alarm v1.3 - Updated Stable Version
// Save this as: ArrearsAlarm.tsx
// 
// FIXES IN THIS VERSION:
// - Error only shows in Upcoming tab (clears on tab change)
// - Error auto-dismisses after 5 seconds
// - Fixed text alignment and overflow issues
// - Guard rail properly checks recurring payment frequency
// - Prevents double-payment of recurring items
// - Added trendy animations and modern styling
    ...
    return (
        <div className="min-h-screen p-4 sm:p-8 bg-gray-900 text-white">
            ...
            <div className="max-w-4xl mx-auto">
                                                        ...
                                                        <div className="flex justify-between items-start">
                                                            <div className="flex-1">
                                                                <div className="flex items-center gap-2 mb-2">
                                                                    ...
                                                                    {payment.isRecurring && (
                                                                        <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-indigo-600/50 text-indigo-200 flex items-center">
                                                                ...
                                                                <p className="text-2xl font-bold text-indigo-300 mb-1">
                                                                    {payment.amount ? `${parseFloat(payment.amount).toFixed(2)}` : 'No amount set'}
                                                                </p>
                                                                
                                                                {reminders.length > 0 && (
                                                                    <div className="mt-3 space-y-1">
                                                                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Reminder Schedule:</p>
                                                                        {reminders.map((reminder, idx) => (
                                                                            <div key={idx} className="flex items-center gap-2 text-sm">
                                                                                <Bell className="w-3 h-3 text-yellow-400" />
                                                                                <span className="text-gray-300">{reminder.label}:</span>
                                                                                <span className="font-semibold text-yellow-300">{reminder.date}</span>
                                                                            </div>
                                                                        ))}
                                                                    </div>
                                                                )}
                ...
                {activeTab === 'History' && (
                    <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 space-y-4">
                        <h2 className="text-2xl font-semibold text-green-400">Paid History</h2>
                        {history.length === 0 ? (
                            <p className="text-gray-500 text-center py-8 italic">No payment history yet</p>
                        ) : (
                            history.map(payment => {
                                const { bg, text } = getCategoryColor(payment.categoryTag);
                                return (
                                    <div key={payment.id} className="p-4 rounded-xl bg-green-900/10 border-l-4 border-green-500">
                                        ...
                                        <p className="text-xl font-bold text-green-300">
                                            {payment.amount ? `${parseFloat(payment.amount).toFixed(2)}` : 'No amount'} - ✓ PAID
                                        </p>
                                        <p className="text-sm text-gray-400">Original due: {payment.dueDate}</p>
                                    </div>
                                );
                            })
                        )}
